...
describeEmbeddedPostgres("issueThreadInteractionService", () => {
  ...
  it("moves agent-authored plan confirmations into review while preserving the return assignee", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const issueId = randomUUID();
    const agentId = randomUUID();
    ...
    const [reviewIssue] = await db.select().from(issues);
    ...
    const idempotent = await interactionsSvc.create({
      id: issueId,
      companyId,
    }, {
      kind: "request_confirmation",
  ...
  it("moves agent-authored questions into review while preserving the return assignee", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const issueId = randomUUID();
    const agentId = randomUUID();
